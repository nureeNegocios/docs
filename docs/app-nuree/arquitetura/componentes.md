...
| **`Account`** | Cadastro / tenancy · *Empresa·Usuário·Programa/Participação·Produto·Perfil* | catálogo de Produtos; Programas (contratos isolados) e matrícula; CRUD empresas/usuários; alternar contexto; convite; link de cadastro aberto e autocadastro; carômetro | [E1.1](../requisitos.md#rf-e1-1)–1.3, E2.1–2.3, 2.5, 2.8–2.11 |
...
| **`Scheduling`** | Agenda · *regras de disponibilidade · slots · sessões · sync · check-in de sessão* | regras de recorrência com vigência, público, cota e intervalo mínimo, e as exceções por ocorrência; materializar slots; agendar sessão (cota da regra e limite do pacote); transferir; registrar combinado; acompanhamento e dimensionamento da oferta; sync Google (OAuth+evento); check-in da sessão (*reusa `Form`*) | [E5.1](../requisitos.md#rf-e5-1)–5.22 |
