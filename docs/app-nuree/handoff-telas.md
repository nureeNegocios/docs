...
Fonte única da marca: `DESIGN.md`. Resumo operável (valores extraídos dos frames de auth):
...
- **Botões:** primário = navy sólido, texto branco; secundário = **fundo branco + borda navy @ 0.16, peso 1** (padrão do "Novo usuário"), nunca borda cheia. Fluxos em etapas trazem um indicador `PASSO x DE y` + barra de progresso no topo (carômetro, criar programa → matricular).
...
- [Requisitos](requisitos.md) · [Ações do sistema](acoes.md) · [Componentes](arquitetura/componentes.md) · [Diagramas](arquitetura/diagramas.md)
